...
def main(args):
    ...
    for i in pbar:
        ...
        img_gen, _ = g_ema([latent], input_is_latent=True, randomize_noise=False)
        cosine_distance_loss = soundclip_loss(img_gen, audio_inputs, audio_inputs_extra, None)
        if args.mode == "edit":
...
if __name__ == "__main__":
    ...
    print ("SOGA!!!")
    torchvision.utils.save_image(result_image.detach().cpu(), os.path.join(args.results_dir, "final_result_wo_text.jpg"), normalize=True, scale_each=True, range=(-1, 1))
