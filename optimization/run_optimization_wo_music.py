...
def main(args):
    ...
    args.latent_path = args.results_dir + "/latent_code.pt"
    ...
    for i in pbar:
        ...
        img_gen, _ = g_ema([latent], input_is_latent=True, randomize_noise=False)
        cosine_distance_loss = soundclip_loss(img_gen, audio_inputs, None, args.description)
        if args.mode == "edit":
        ...
        if args.save_intermediate_image_every > 0 and i % args.save_intermediate_image_every == 0:
            with torch.no_grad():
                img_gen, _ = g_ema([latent], input_is_latent=True, randomize_noise=False)
            torchvision.utils.save_image(img_gen, "results_wo_music" + f"/{str(i).zfill(5)}.jpg", normalize=True, range=(-1, 1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="results_wo_music")
    ...
    torchvision.utils.save_image(result_image.detach().cpu(), os.path.join(args.results_dir, "final_result_wo_music.jpg"), normalize=True, scale_each=True, range=(-1, 1))
