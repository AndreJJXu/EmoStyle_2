...
from criteria.cliip_loss import SoundCLIPLoss
...
def main(args):
    ...
    for i in pbar:
        ...
        img_gen, _ = g_ema([latent], input_is_latent=True, randomize_noise=False)
        cosine_distance_loss = clip_loss(img_gen, args.description)
        if args.mode == "edit":
...
if __name__ == "__main__":
    ...
    print("StyleCLIP!!!")
    print(args.results_dir, "final_result_StyleCLIP.jpg")
    torchvision.utils.save_image(result_image.detach().cpu(), os.path.join(args.results_dir, "final_result_StyleCLIP.jpg"), normalize=True, scale_each=True, range=(-1, 1))
