...
def main(args):
    ...
    if args.latent_path:
        ...
        clip_model, _ = clip.load("/mnt/ssd/BeautifulXJJ/AIGC/Sound-Image-Generation/sound-guided-semantic-image-manipulation/pretrained_models/ViT-B-32.pt", device=device)

        generated_image = Image.open('/mnt/ssd/BeautifulXJJ/AIGC/Sound-Image-Generation/sound-guided-semantic-image-manipulation/chain.png')
        generated_image = generated_image.resize((224, 224))
        
        image_tensor = ToTensor()(generated_image).to(device)
        # print("generated_image size: ", image_tensor.size())
        # image_test = g_ema.get_latent(image_tensor)
        ...
        image_embedding = clip_model.encode_image(image_tensor).float()

        torch.save(image_embedding, 'latent_code.pt')
        latent_code_init = torch.load(args.latent_path).cuda()

        latent_code_init = torch.tensor(latent_code_init).to(device)
        # latent_code_init = latent_code_init.detach().clone().repeat(1, 18, 1)


        with torch.no_grad():
            img_orig, latent_code_init = g_ema([latent_code_init], return_latents=True,
                                        truncation=args.truncation, truncation_latent=mean_latent)
            # img_orig, latent_code_init = g_ema([latent_code_init], return_latents=True, input_is_latent=True, randomize_noise=False)
        img_orig = image_tensor
...
# python3 optimization/run_optimization.py --lambda_similarity 0.002 --lambda_identity 0.0 --truncation 0.7 --lr 0.1 --audio_path "./audiosample/explosion.wav" --ckpt ./pretrained_models/landscape.pt --stylegan_size 256 --latent_path "latent_code.pt"
