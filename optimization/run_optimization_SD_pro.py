...
def main(args):
    ...
    # device = "cuda"
    pipe = StableDiffusionPipeline.from_pretrained(args.ckpt, torch_dtype=torch.float16)
    pipe = pipe.to(device)
    ...
    if args.latent_path:
        ...
        with torch.no_grad():
            img_orig = pipe(prompt = args.description, latents = latent_code_init).images
            img_trans = img_orig.transpose(0, 3, 1, 2)
            img_orig_final = torch.from_numpy(img_trans)

    elif args.mode == "edit":
        for i in range(args.num_pic):
            latent_code_init_not_trunc = torch.randn((1, 4, 64, 64), dtype=torch.float16).cuda()
            print("edit: ", latent_code_init_not_trunc.shape)
            with torch.no_grad():
                img_orig = pipe(prompt = args.description, latents = latent_code_init_not_trunc, output_type = 'np').images
                # img_orig.convert('RGB')
                img_trans = img_orig.transpose(0, 3, 1, 2)
                img_tensor = torch.from_numpy(img_trans)
                # print(img_tensor.shape)
            #store gen img
            torchvision.utils.save_image(img_tensor, f"img_ph1/{str(i)}.jpg", normalize=True, range=(-1, 1))
            img_path_tmp = f"img_ph1/{str(i)}.jpg"
            img_paths.append(img_path_tmp)
            image_list.append(img_tensor)
            latent_code_inits.append(latent_code_init_not_trunc)
        ...
        img_orig_final = image_list[max_index].cuda()
        latent_code_init = latent_code_inits[max_index].cuda()
        with torch.no_grad():
            img_orig = pipe(prompt = args.description, latents = latent_code_init, output_type = 'np').images
            # img_orig.convert('RGB')
            img_trans = img_orig.transpose(0, 3, 1, 2)
            img_tensor = torch.from_numpy(img_trans)
            torchvision.utils.save_image(img_tensor, f"img_ph1/{str(666)}.jpg", normalize=True, range=(-1, 1))
    ...
    with torch.no_grad():
        img_orig = pipe(prompt = args.description, latents = latent, output_type = 'np').images
        # img_orig.convert('RGB')
        img_trans = img_orig.transpose(0, 3, 1, 2)
        img_tensor = torch.from_numpy(img_trans)
        torchvision.utils.save_image(img_tensor, f"img_ph1/{str(999)}.jpg", normalize=True, range=(-1, 1))
    latent_add = torch.randn((1, 1, 1, 64), dtype=torch.float16).cuda()
    latent_add.requires_grad = True
    soundclip_loss = SoundCLIPLoss(args)
    id_loss = IDLoss(args)
    optimizer = optim.Adam([latent_add], lr=args.lr)
    ...
    for i in pbar:
        ...
        img_gen = pipe(prompt = args.description, latents = latent, output_type = 'np').images
        img_trans = img_gen.transpose(0, 3, 1, 2)
        img_tensor_gen = torch.from_numpy(img_trans).cuda()
        torchvision.utils.save_image(img_tensor_gen, f"img_ph2/{str(i)}.jpg", normalize=True, range=(-1, 1))
        cosine_distance_loss = soundclip_loss(img_tensor_gen, audio_inputs)
        if args.mode == "edit":
            ...
            loss = args.lambda_similarity * similarity_loss + cosine_distance_loss  + args.lambda_identity * id_loss(img_orig_final, img_tensor_gen)[0]
        ...
        # for name, param in latent_add.named_parameters():
        #     if param.grad is not None:
        #         print(f"Layer: {name}, Gradient norm: {param.grad.norm().item()}")
        nn.utils.clip_grad_norm_(latent_add, 1.0)
        optimizer.step()
        ...
        latent = latent * 0.99 + latent_add * 0.01
        if args.save_intermediate_image_every > 0 and i % args.save_intermediate_image_every == 0:
            
            with torch.no_grad():
                img_gen= pipe(prompt = args.description, latents = latent, output_type = 'np').images
                img_trans = img_gen.transpose(0, 3, 1, 2)
                img_gen_tensor = torch.from_numpy(img_trans)
            torchvision.utils.save_image(img_gen_tensor, f"results/{str(i).zfill(5)}.jpg", normalize=True, range=(-1, 1))
    
    if args.mode == "edit":
        with torch.no_grad():
            img_orig = pipe(prompt = args.description, latents = latent_code_init, output_type = 'np').images
            img_trans = img_orig.transpose(0, 3, 1, 2)
            img_orig_tensor = torch.from_numpy(img_trans)
        final_result = torch.cat([img_orig_tensor, img_gen_tensor])
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default="/mnt/ssd/BeautifulXJJ/AIGC/Sound-Image-Generation/Diffusion_gen/SD-v1-4/stable-diffusion-v1-4", help="pretrained StyleGAN2 weights")
